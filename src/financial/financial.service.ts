...
export class FinancialService {
    ...
    async saveFinancialOperation(operations: FinancialOperation[]): Promise<{ totalInserted: number, failedOperations: { operation: FinancialOperation, reason: string }[] }> {
        const suspiciousList = operations.map((operation) => this.evaluateOperationSuspicion(operation))

        const result = await this.repository.saveFinancialData(operations, suspiciousList)
